...
class APIClient {
    ...
    async signup(data: { email: string; username: string; password: string }) {
        const response = await this.client.post('/users/open', data);
        return response.data;
    }
    ...
    async createProject(data: { name: string; description?: string; domain?: string }) {
    ...
    async updateProject(projectId: string, data: { name?: string; description?: string; status?: string; domain?: string }) {
        const response = await this.client.put(`/projects/${projectId}`, data);
        return response.data;
    }

    // Deployment endpoints
    async getDeployments(projectId: string, params?: { environment_id?: string; skip?: number; limit?: number }) {
    ...
    async createEnvironment(data: { name: string; subdomain_prefix: string; description?: string }) {
    ...
    async updateEnvironment(environmentId: string, data: { name?: string; subdomain_prefix?: string; description?: string }) {
        const response = await this.client.put(`/environments/${environmentId}`, data);
        return response.data;
    }
